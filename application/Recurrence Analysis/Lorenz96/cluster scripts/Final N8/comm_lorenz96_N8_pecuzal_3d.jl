...
writedlm("results_Lorenz96_N_$(N)_pec_3d_params.csv", params)
writedlm("results_Lorenz96_N_$(N)_pec_3d_Fs.csv", Fs)

varnames = ["tau_pec", "ts_pec", "optimal_d_pec", "RQA_pec", "L_pec", "RP_frac_pec"]

for i = 1:length(varnames)
    writestr = "results_Lorenz96_N_$(N)_pec_3d_"*varnames[i]*".csv"
    data = []
