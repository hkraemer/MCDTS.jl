...
results = @distributed (vcat) for i in eachindex(Fs)
    ...
    for i = 1:3
        ...
        RQA_tde[:,i] = hcat(RQA...)
        L_tde[i] = uzal_cost(regularize(𝒟); w = τ_tde[i], samplesize=1, Tw = τ_tde[1])
    end
    Tw = τ_tde[1]
    ...
    # MCDTS
    tree = MCDTS.mc_delay(Dataset(data_sample), theiler, (L)->(MCDTS.softmaxL(L,β=2.)), taus, trials; Tw = Tw)
