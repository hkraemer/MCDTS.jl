...
begin
    x1 = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/x1.csv"))
    x2 = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/x2.csv"))
    y1 = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/y1.csv"))
    y2 = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/y2.csv"))
    x1_n = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/x1_n.csv"))
    x2_n = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/x2_n.csv"))
    y1_n = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/y1_n.csv"))
    y2_n = vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/y2_n.csv"))

    data_sample = Dataset(hcat(x1,y1))
    data_sample_n = Dataset(hcat(x1_n,y1_n))
    tr = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tr.csv"))
    τ_cao = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_cao.csv")))
    τ_cao_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_cao_n.csv")))
    Y_cao = genembed(x1, τ_cao .* (-1))
    Y_cao_n = genembed(x1_n, τ_cao_n .* (-1))
    τ_kennel = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_kennel.csv")))
    τ_kennel_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_kennel_n.csv")))
    Y_kennel = genembed(x1, τ_kennel .* (-1))
    Y_kennel_n = genembed(x1_n, τ_kennel_n .* (-1))
    τ_hegger = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_hegger.csv")))
    τ_hegger_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_hegger_n.csv")))
    Y_hegger = genembed(x1, τ_hegger .* (-1))
    Y_hegger_n = genembed(x1_n, τ_hegger_n .* (-1))
    τ_pec = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_pec.csv")))
    ...
    ts_pec2 = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_pec2.csv")))
    ts_pec2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_pec2_n.csv")))
    Y_pec2 = genembed(data_sample, τ_pec2 .* (-1), ts_pec2)
    Y_pec2_n = genembed(data_sample_n, τ_pec2_n .* (-1), ts_pec2_n)
    τ_mcdts2 = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts2.csv")))
    ts_mcdts2 = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_mcdts2.csv")))
    Y_mcdts2 = genembed(data_sample, τ_mcdts2 .* (-1), ts_mcdts2)
    τ_mcdts_fnn = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_fnn.csv")))
    Y_mcdts_fnn = genembed(x1, τ_mcdts_fnn .* (-1))
    τ_mcdts_fnn2 = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_fnn2.csv")))
    ts_mcdts_fnn2 = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_mcdts_fnn2.csv")))
    Y_mcdts_fnn2 = genembed(data_sample, τ_mcdts_fnn2 .* (-1), ts_mcdts_fnn2)
    τ_mcdts_fnn_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_fnn_n.csv")))
    Y_mcdts_fnn_n = genembed(x1_n, τ_mcdts_fnn_n .* (-1))
    τ_mcdts_fnn2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_fnn2_n.csv")))
    ts_mcdts_fnn2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_mcdts_fnn2_n.csv")))
    Y_mcdts_fnn2_n = genembed(data_sample, τ_mcdts_fnn2 .* (-1), ts_mcdts_fnn2)
    # τ_mcdts2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts2_n.csv")))
    ...
    # τ_mcdts_PRED_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_PRED_n.csv")))
    # Y_mcdts_PRED_n = genembed(x1_n, τ_mcdts_PRED_n .* (-1))
    # τ_mcdts_PRED2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/tau_mcdts_PRED_multi_n.csv")))
    # ts_mcdts_PRED2_n = Int.(vec(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Results 1/ts_mcdts_PRED_multi_n.csv")))
...
begin
    ...
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Cao_zeroth.csv",Cao_zeroth)
    ## Cao_zeroth_n = MCDTS.iterated_local_zeroth_prediction(Y_cao_n, KK, T_steps; theiler = w1_n, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Cao_zeroth_n.csv",Cao_zeroth_n)
    ## println("Kennel")
    ## println("*****")
    ## Kennel_zeroth = MCDTS.iterated_local_zeroth_prediction(Y_kennel, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Kennel_zeroth.csv",Kennel_zeroth)
    ## Kennel_zeroth_n = MCDTS.iterated_local_zeroth_prediction(Y_kennel_n, KK, T_steps; theiler = w1_n, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Kennel_zeroth_n.csv",Kennel_zeroth_n)
    ## println("Hegger")
    ## println("*****")
    ## Hegger_zeroth = MCDTS.iterated_local_zeroth_prediction(Y_hegger, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Hegger_zeroth.csv",Hegger_zeroth)
    ## Hegger_zeroth_n = MCDTS.iterated_local_zeroth_prediction(Y_hegger_n, KK, T_steps; theiler = w1_n, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Hegger_zeroth_n.csv",Hegger_zeroth_n)
    ## println("Pec")
    ## println("*****")
    ## Pec_zeroth = MCDTS.iterated_local_zeroth_prediction(Y_pec, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Pec_zeroth.csv",Pec_zeroth)
    ## Pec_zeroth2 = MCDTS.iterated_local_zeroth_prediction(Y_pec2, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Pec_zeroth2.csv",Pec_zeroth2)
    ## Pec_zeroth_n = MCDTS.iterated_local_zeroth_prediction(Y_pec_n, KK, T_steps; theiler = w1_n, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Pec_zeroth_n.csv",Pec_zeroth_n)
    ...
    # writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/mcdts_zeroth_n.csv",mcdts_zeroth_n)
    # mcdts_zeroth2_n = MCDTS.iterated_local_zeroth_prediction(Y_mcdts2_n, KK, T_steps; theiler = w1_n, verbose=true)
    # writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/mcdts_zeroth2_n.csv",mcdts_zeroth2_n)
    ## println("mcdts FNN:")
    ## println("*****")
    ## mcdts_fnn_zeroth = MCDTS.iterated_local_zeroth_prediction(Y_mcdts_fnn, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/mcdts_fnn_zeroth.csv",mcdts_fnn_zeroth)
    ## mcdts_fnn_zeroth2 = MCDTS.iterated_local_zeroth_prediction(Y_mcdts_fnn2, KK, T_steps; theiler = w1, verbose=true)
    ## writedlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/mcdts_fnn_zeroth2.csv",mcdts_fnn_zeroth2)
...
# load data
load = begin
    PREDICTIONS[1,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Cao_zeroth.csv"))[:,1]
    PREDICTIONS_n[1,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Cao_zeroth_n.csv"))[:,1]
    PREDICTIONS[2,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Kennel_zeroth.csv"))[:,1]
    PREDICTIONS_n[2,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Kennel_zeroth_n.csv"))[:,1]
    PREDICTIONS[3,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Hegger_zeroth.csv"))[:,1]
    PREDICTIONS_n[3,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Hegger_zeroth_n.csv"))[:,1]
    PREDICTIONS[4,:] = Dataset(readdlm("./application/artificial data/Lorenz Prediction/2nd Trial/Prediction Results/Pec_zeroth.csv"))[:,1]
