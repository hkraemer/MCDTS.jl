...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    # mcdts PRED MSE
    MSEs_mcdts_PRED_KL = zeros(T_steps)
    ...
    for j = 1:T_steps
        MSEs_mcdts_PRED_KL[j] = MCDTS.compute_mse(prediction[1:j,1], x2[1:j]) / σ₂
    end

    MSEs_mcdts_PRED_KL_n = zeros(T_steps)
    ...
    for j = 1:T_steps
        MSEs_mcdts_PRED_KL_n[j] = MCDTS.compute_mse(prediction[1:j,1], x2_n[1:j]) / σ₂_n
    end

    # Output
    tuple(MSEs_mcdts_PRED_KL, MSEs_mcdts_PRED_KL_n)
...
varnames = ["MSEs_mcdts_PRED_KL", "MSEs_mcdts_PRED_KL_n"]
