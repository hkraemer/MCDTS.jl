...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    for j = 1:T_steps
        MSEs_mcdts_L[j] = MCDTS.compute_mse(prediction[1:j,1], x2[1:j]) / σ₂
    end

    MSEs_mcdts_L_n = zeros(T_steps)
    ...
    for j = 1:T_steps
        MSEs_mcdts_L_n[j] = MCDTS.compute_mse(prediction[1:j,1], x2_n[1:j]) / σ₂_n
    end


    # Output
    tuple(MSEs_mcdts_L, MSEs_mcdts_L_n)
...
varnames = ["MSEs_mcdts_L", "MSEs_mcdts_L_n"]
