...
# mcdts
trials = 80
tree = MCDTS.mc_delay(Dataset(x1_n), w1_n, (L)->(MCDTS.softmaxL(L,β=2.)), taus, trials; FNN=true, threshold = 0.05, verbose = true)
best_node = MCDTS.best_embedding(tree)
τ_mcdts_fnn_n = best_node.τs
Y_mcdts_fnn_n = genembed(x1_n, τ_mcdts_fnn_n .* (-1))

# Save data
writedlm("Y_mcdts_fnn_n.csv", Y_mcdts_fnn_n)
writedlm("tau_mcdts_fnn_n.csv", τ_mcdts_fnn_n)
