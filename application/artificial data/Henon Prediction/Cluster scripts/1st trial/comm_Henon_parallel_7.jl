...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    # mcdts PRED MSE
    MSEs_mcdts2_PRED = zeros(T_steps)
    ...
    for j = 1:T_steps
        MSEs_mcdts2_PRED[j] = MCDTS.compute_mse(prediction[1:j,tts], x2[1:j]) / σ₂
    end

    MSEs_mcdts2_PRED_n = zeros(T_steps)
    ...
    for j = 1:T_steps
        MSEs_mcdts2_PRED_n[j] = MCDTS.compute_mse(prediction[1:j,tts], x2_n[1:j]) / σ₂_n
    end

    # Output
    tuple(MSEs_mcdts2_PRED, MSEs_mcdts2_PRED_n)
...
varnames = ["MSEs_mcdts2_PRED", "MSEs_mcdts2_PRED_n"]
