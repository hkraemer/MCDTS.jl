...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    # mcdts PRED L
    MSEs_mcdts2_PRED_L_KL_n = zeros(T_steps)
    ...
    Y = genembed(x1_n, τ_mcdts_n .* (-1), ts_mcdts_n)
    prediction = MCDTS.iterated_local_zeroth_prediction(Y, KK, T_steps; theiler = w1_n)
    for j = 1:T_steps
        MSEs_mcdts2_PRED_L_KL_n[j] = MCDTS.compute_mse(prediction[1:j,1], x2_n[1:j]) / σ₂_n
    end


    # Output
    tuple(MSEs_mcdts2_PRED_L_KL_n)
...
varnames = ["MSEs_mcdts2_PRED_L_KL_n"]
