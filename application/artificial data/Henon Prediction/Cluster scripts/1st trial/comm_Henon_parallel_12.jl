...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    Y = genembed(x1, τ_mcdts_n .* (-1))
    prediction = MCDTS.iterated_local_zeroth_prediction(Y, KK, T_steps; theiler = w1)
    for j = 1:T_steps
        MSEs_mcdts_PRED_L_KL[j] = MCDTS.compute_mse(prediction[1:j,1], x2[1:j]) / σ₂
    end
    ...
    prediction = MCDTS.iterated_local_zeroth_prediction(Y, KK, T_steps; theiler = w1)
    for j = 1:T_steps
        MSEs_mcdts2_PRED_L_KL[j] = MCDTS.compute_mse(prediction[1:j,1], x2[1:j]) / σ₂
    end


    # Output
    tuple(MSEs_mcdts_PRED_L_KL, MSEs_mcdts2_PRED_L_KL)
...
varnames = ["MSEs_mcdts_PRED_L_KL", "MSEs_mcdts2_PRED_L_KL"]
