...
results = @distributed (vcat) for i in eachindex(ics)
    ...
    for j = 1:T_steps
        MSEs_mcdts_FNN[j] = MCDTS.compute_mse(prediction[1:j,1], x2[1:j]) / σ₂
    end

    MSEs_mcdts_FNN_n = zeros(T_steps)
    tree = MCDTS.mc_delay(Dataset(x1_n), w1_n, (L)->(MCDTS.softmaxL(L,β=2.)), taus1, trials1; FNN=true, threshold = 0.05, verbose=true)
    best_node = MCDTS.best_embedding(tree)
    ...
    for j = 1:T_steps
        MSEs_mcdts_FNN_n[j] = MCDTS.compute_mse(prediction[1:j,1], x2_n[1:j]) / σ₂_n
    end


    # Output
    tuple(MSEs_mcdts_FNN, MSEs_mcdts_FNN_n)
...
varnames = ["MSEs_mcdts_FNN", "MSEs_mcdts_FNN_n"]
