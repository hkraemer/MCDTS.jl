...
function pecora(
        ...
        p::T = 0.5, PRED::Bool = false) where {D, T<:Real}
    ...
    L = length(vspace)
    if PRED
        ns = vec((1+maximum(abs.(delays))):L)
    else
        ns = vec(1:(L-maximum(abs.(delays))))
    end
    ...
    for i in 1:length(J)
        ...
        all_ε★[:, i] .= continuity_per_timeseries(x, ns, allNNidxs, delays, K, α, p; PRED = PRED)
        if undersampling
